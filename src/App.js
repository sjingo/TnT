...
const LoginForm = () => {
  ...
  const checkForErrors = () => !state.nameError || !state.emailError || !state.numberInParty === ''
  const handleSubmit = () =>{
  ...
  if(isValid){
    ...
    db.collection('TrackAndTrace').doc('Hagglers').update({
      [`${today}.${name}_${timeStamp}`]: {name,mobile,email,numberInParty}
     }).then(handlSuccess).catch(handleFormError)
    return true
  } 
  setFormError(true)
  return false 
  ...
  const handleChange = (e,data) =>{
   ...
   <Message  success={formSucces} ><Message.Header>Covid track and trace</Message.Header>We'll only use data from this form for Covid track and trace purposes. If you'd like to <Popup size="large" inverted content="Hey, so the thing is... we're still working on the app. And it's not quite ready yet. We'll let people know, as soon as its fully baked" trigger={<a href='#'>download the app</a>}/> - you can checkin with one click next time and keep up with hagglers' goings on. </Message>
        ...
        <Transition visible={!formSucces} animation='fade' duration={500}>
          ...
          <Form size='large' onSubmit={handleSubmit} error={formError} success={formSucces}>
            <Segment stacked>
                
                <Form.Input error={formError && state.nameError || state.nameError && state.touched.name} name={NAME} onChange={handleChange} fluid icon='user' iconPosition='left' placeholder='Name' />
                <Form.Input  name={EMAIL} error={formError && state.emailError || state.emailError && state.touched.email} onChange={handleChange} fluid icon='mail' iconPosition='left' placeholder='E-mail address' />
                <Form.Input  name={MOBILE} onChange={handleChange} fluid icon='mobile' iconPosition='left' placeholder='Mobile' />
                <Form.Input  name={NUMBER_IN_PARTY} onChange={handleChange}  min={0} max={6} fluid icon='users' iconPosition='left' placeholder='Number in party' type="number" />

                {/* <Form.Checkbox  label='remember me on this device for 90 days' placeholder='Mobile' /> */}

              <Button color='teal' fluid size='large' disabled={formError}>
                Save
              </Button>
            </Segment>
          </Form>
